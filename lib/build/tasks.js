...
const timeUtils = require(__dirname+'/../converter/timeutils.js')
	...
	, { CustomPages, Stats, Posts, Files, Boards, News, Modlogs, Blogs } = require(__dirname+'/../../db/')
...
module.exports = {
	...
	buildActiveThreads: async (options) => {
		const label = '/activethreads.html';
		const start = process.hrtime();
		options.managePage = 'activethreads.html';
		const threads = await Posts.getActiveThreads(options.userSession);
		const { html, json } = await render(label, 'activethreads.pug', {
			...options,
			threads,
		}, {
			'name': '/activethreads.json',
			'data': threads
		});
		const end = process.hrtime(start);
		debugLogs && console.log(timeDiffString(label, end));
	...
	buildMyThreads: async (options) => {
		const label = '/mythreads.html';
		const start = process.hrtime();
		options.managePage = 'mythreads.html';
		const threads = await Posts.getMyThreads(options.userSession);
		const { html, json } = await render(label, 'mythreads.pug', {
			...options,
			threads,
		}, {
			'name': '/mythreads.json',
	...
	buildBoard: async (options) => {
		...
		let blogs = [];
		if (options.board._id === 'gen' && options.page === 1) {
			blogs = await Blogs.getAll(options.board._id);
			// Migrate old blog format
	...
	buildHomepage: async () => {
		...
		const start = process.hrtime();
		let [ totalStats, boards, fileStats, recentNews, featuredThreads, recentFiles, totalPosts, newsCount, recentPosts, postsToday, activeUsers ] = await Promise.all([
			...
			Posts.getFeaturedThreads(10), //admin featured threads
			Posts.getRecentFiles(20), //recent files/images with post info
			Posts.totalPosts(), //actual total posts from posts collection
			News.db.countDocuments(), //news posts count
			Posts.getRecentWithReplies(15, false), //recent posts including replies across all boards
			Posts.postsToday(), //posts made in the last 24 hours
			Posts.activeUsers(), //unique users who posted in the last hour
		]);
		const [ localStats, webringStats ] = totalStats;
		// Override posts count with actual count from posts collection + news
		localStats.posts = totalPosts + newsCount;

		// Sort boards in custom order
		const customOrder = ['a', 'b', 'pol', 'g', 'gen', 'life', 'ent', 'x', 'kama', 'meta'];
		boards.sort((a, b) => {
