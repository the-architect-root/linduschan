...
module.exports = {
	...
	buildHomepage: async () => {
		...
		const start = process.hrtime();
		let [ totalStats, boards, fileStats, recentNews, hotThreads, recentFiles, totalPosts, newsCount, recentPosts ] = await Promise.all([
			...
			Files.getRecent(10), //recent files/images
			Posts.totalPosts(), //actual total posts from posts collection
			News.db.countDocuments(), //news posts count
			Posts.getRecentWithReplies(5, false), //recent posts including replies across all boards
		]);
