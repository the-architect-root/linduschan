	...
	if (req.session && res.locals && res.locals.user) {
		...
		if (res.locals.board != null) {
			if (res.locals.board.owner === user.username) {
				...
				boardPermissions = res.locals.board.staff[user.username] 
					? new Permission(res.locals.board.staff[user.username].permissions)
					: new Permission(roleManager.roles.BOARD_OWNER.base64);
