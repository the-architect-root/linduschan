...
const updateMarkdownPerms = () => {
	replacements = [
		...
		{ permission: Permissions.USE_MARKDOWN_TITLE, regex: titleRegex, cb: (permissions, match, title) => `<span class='title-xl'>${title}</span>` },
		{ permission: Permissions.USE_MARKDOWN_TITLE, regex: titleLgRegex, cb: (permissions, match, title) => `<span class='title'>${title}</span>` },
		{ permission: Permissions.USE_MARKDOWN_TITLE, regex: titleSmRegex, cb: (permissions, match, title) => `<span class='title-sm'>${title}</span>` },
...
module.exports = {
	...
	markdown: (text, permissions, boardName=null, postId=null) => {
		...
		for (let i = 0; i < chunks.length; i++) {
			...
			if (i % 2 === 0) {
				...
				// Replace BOARD and POSTID placeholders with actual values
				let processed = newlineFix;
				if (boardName) {
					processed = processed.replace(/:BOARD:/g, `:${boardName}:`);
				}
				if (postId) {
					processed = processed.replace(/:POSTID:/g, `:${postId}:`);
				}
				chunks[i] = module.exports.processRegularChunk(processed, permissions);
