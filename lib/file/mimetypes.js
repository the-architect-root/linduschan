...
async function realMimeCheck(file) {
	...
	if (realMimeType) {
		...
		file.extension = `.${realMimeType.ext}`;
		file.realMimetype = realMimeType.mime;
		//update the mimetype to the real one for subsequent checks
		file.mimetype = realMimeType.mime;
		...
		// Check if the real mimetype is allowed (in any of the sets)
		const isAllowed = image.has(realMimeType.mime) ||
			animatedImage.has(realMimeType.mime) ||
			video.has(realMimeType.mime) ||
			audio.has(realMimeType.mime) ||
			other.has(realMimeType.mime);

		// If the real type is allowed, always accept it regardless of extension mismatch
		if (isAllowed) {
			return true;
		}

		// Otherwise check for exact match or equivalent
