		...
		thumbnailing.resize(Math.min(thumbSize, file.geometry.width), Math.min(thumbSize, file.geometry.height))
			...
			.write(`${uploadDirectory}/file/thumb/${file.hash}${file.thumbextension}`, function (err) {
				if (err) {
					...
					console.warn('ImageMagick not available, using original as thumbnail');
					const fs = require('fs-extra');
					fs.copy(`${uploadDirectory}/file/${file.filename}`, `${uploadDirectory}/file/thumb/${file.hash}${file.thumbextension}`)
						.then(() => resolve())
						.catch(reject);
					return;
				}
